...
def drive_distance(x, y, new_x, new_y ):
    """
    Calculates drive distance because assholes said so
    ...
    :return: float, drive distance
    """
    return sqrt((new_x - x) ** 2 + (new_y - y) ** 2)

def calculate_new_position(x, y, new_x, new_y, gas, gas_consumption):
    """
    ...
    :param new_x: float, the new x location
    :param new_y: float, the new y location
    :param gas: float, the ammunt of gas in the tank
    ...
    """
    # we calculate how far the point we need to move to
    m = drive_distance(x, y, new_x, new_y)
    # we calculate how far we can drive
    p = (gas * 100) / gas_consumption
    ...
    if p < m:
        ...
        gas = gas - gas_consumption / 100 * m
    return new_x, new_y, gas
...
def drive(x, y, new_x, new_y, gas, gas_consumption):
    ...
    input.
    """
    x, y, gas = calculate_new_position(x, y, new_x, new_y, gas, gas_consumption)
    return gas, x, y
